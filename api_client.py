...
# 캐시 수정 시각이 이만큼까지 미래면 '방금 쓴 것'(나이 0)으로 본다. 파일 시각은 거친
# 시스템 틱, time.time() 은 정밀 시계라 쓰자마자 읽으면 몇 ms 미래가 나온다 — CI 러너에서
# 실제로 그래서 신선한 캐시를 버리고 다시 요청했다. 가장 짧은 TTL(15분)보다 훨씬 작게 둔다:
# 이보다 먼 미래(시계를 되돌림·다른 PC 에서 복사)는 여전히 나이를 모르는 것으로 본다(F-012).
MTIME_SKEW_TOLERANCE = 60
