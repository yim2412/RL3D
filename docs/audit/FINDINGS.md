...
- 결과: 스모크가 `check_update("0.0.0", force=True)` — 캐시를 우회한다(GitHub 1회). CLAUDE.md 릴리스 절에 한 줄.
...
- 결과: `year_raw = year` 를 docstring 뒤로. `get_archive.__doc__` 이 다시 문자열(실측).
...
- 결과: 주석을 '테스트 전용 기준 구현 — 앱은 나눠 도는 경로만 쓴다'로.
...
- 결과: `_clean_groups` 가 문자열 원소만 받는다. 회귀 `TestAuditLowFixes`. 되돌리는 변이 FAIL.
...
- 결과: 창 복원을 순수 함수 `restored_geometry` 로 떼고 `window` 가 dict 가 아니면 기본값(bool 크기도 거절). 회귀 `TestRestoredGeometry` 6건.

### F-012 · 영역: 시각 경계 · 상태: 완료
...
- 결과: 음수 나이는 '모름'(None)으로 — 신선 판정에서 빠진다. 회귀 1건. 되돌리는 변이 FAIL.
- 후속(2026-09-24): **이 수정이 CI 를 빨갛게 만들었다**(ca1a834·5bcec63 두 번, 테스트는 매번 달랐다 — 증상은 같다: 방금 쓴 캐시를 버리고 다시 요청). windows-latest 러너에서 쓰자마자 읽은 수정 시각이 ms 단위로 미래였다(로컬 2,000회 중 0회 — 재현 안 됨). '음수'가 아니라 '허용 폭(`MTIME_SKEW_TOLERANCE` 60초)보다 먼 미래'만 모름으로 본다. 회귀 1건(+0.05초 미래 → 캐시 사용). 변이 2건 FAIL: 허용 폭 0 → 새 테스트 · 10일 → 기존 F-012 테스트. **CI 를 푸시 뒤에 안 봐서 두 커밋 동안 몰랐다.**
- 후속 2(2026-09-24): **그 수정도 불완전했다** — 8893d9d CI 빨강(강제 갱신 요청 4≠2). 허용 폭 안의 음수 나이를 그대로 두면 TTL 비교는 통과하지만 강제 갱신 문턱 `0 <= age` 에서 빠진다. 후속 1 에서 `elif age < 0: age = 0` 을 "없어도 결과가 같다"며 뺐는데 **TTL 비교만 보고 내린 판단**이었다. 되살리고 회귀 1건(+0.05초 미래 → 강제 갱신도 캐시 사용), 수정 전 로컬 FAIL 확인 · 새 줄을 끄는 변이 FAIL. 로컬에서는 시각 차이가 안 나 `ci_local` 도 초록이었다 — **재현을 시각 주입으로 만들지 않으면 CI 에서만 보인다.** (v1.64.2)
...
- 결과: `next` 가 `LL2_BASE/` 로 시작하지 않으면 따라가지 않고 잘린 것으로 표시. 기존 테스트의 가짜 next(`https://ll/…`) 6곳을 LL2_BASE 로 바꿈. 회귀 1건. 되돌리는 변이 FAIL.
...
- 결과: 18줄 모두 **비동치**로 판정 — 정상 입력에서 결과가 바뀐다(빈 배열 가드를 뒤집으면 중계·소식·종류 구역이 **있을 때 오히려 사라진다** · `tzName`·`zoneShortName` 이 빈 문자열 · 소유국 통계 `byCountry` 는 집계만 되고 단언이 0개였다). 착수 시 격리 사본에서 다시 뒤집어 18/18 생존을 먼저 확인(대조군 focus.js:100 은 잡힘). test_frontend 에 26건, 하한 1443 → 1469. 재변이 18/18 FAIL. 처음 쓴 `applySatFilter` 단언은 두 경우의 호출 수를 **합쳐** 세어 뒤집어도 합이 같았다(생존 1) — 경우별로 나눠 고쳤다.
...
- 결과: F-011 의 `restored_geometry` 로 판정을 테스트가 닿게 했다. main.py if-py 변이: 새 함수의 조건은 전부 잡힘 · 남은 10줄은 개발 모니터 선택(89~104)·로그·`_run()` 배선, 그리고 test_cache 가 따로 재는 open_url·로그 레벨(이번 명령엔 test_parsing 만 넣었다).
...
- 결과: 원인 실측 — 100회 중 4회 실패, 전부 같은 테스트(test_readers_never_see_an_empty_file_while_writing). 계측하니 저장의 os.replace 와 겹친 읽기가 PermissionError 3연속으로 재시도(60ms)를 소진해 {} 를 돌려줬다. 테스트 흔들림이 아니라 **제품 결함**(브릿지 스레드끼리 조회·저장이 겹치면 설정이 비어 보임). load_settings 도 _settings_lock 을 거치게 해 80라운드 빈 읽기 4→0.
